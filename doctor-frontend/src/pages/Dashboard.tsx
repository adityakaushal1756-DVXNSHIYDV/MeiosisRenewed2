...
export default function Dashboard(props: DashboardProps) {
  ...
  // ── Clinic Identity (for PDF generation) ──────────────────────
  ...
  const [clinicIdentitySaving, setClinicIdentitySaving] = useState(false);
  const [clinicIdentityToast, setClinicIdentityToast] = useState<{ ok: boolean; msg: string } | null>(null);
  ...
  const settingsView = (
    <div className="grid gap-5 xl:grid-cols-[1fr_1fr]">
      <div className="space-y-5">
        ...
        {/* ── Clinic Identity Panel ── */}
        <PlaceholderBlock
          title="Clinic Identity"
          copy="These details appear on every printed prescription PDF — clinic name top-left bold, contact info top-right. Saved to your doctor profile in the database."
        >
          <div className="space-y-4">
            {/* Toast */}
            {clinicIdentityToast && (
              <div className={`flex items-center gap-2 rounded-2xl border px-4 py-3 text-sm ${
                clinicIdentityToast.ok
                  ? 'border-neon/25 bg-neon/8 text-neon'
                  : 'border-red-400/25 bg-red-400/8 text-red-300'
              }`}>
                {clinicIdentityToast.ok ? '✓' : '✗'} {clinicIdentityToast.msg}
              </div>
            )}
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div className="sm:col-span-2">
                <label className="mb-1.5 block text-xs font-semibold uppercase tracking-[0.2em] text-mist">
                  Clinic Name
                </label>
                <input
                  id="clinicNameInput"
                  type="text"
                  value={clinicIdentity.clinicName}
                  onChange={e => setClinicIdentity(p => ({ ...p, clinicName: e.target.value }))}
                  placeholder="e.g. Apollo Clinic, Greenfield Medical Centre"
                  className="w-full rounded-2xl border border-wire/15 bg-white/[0.04] px-4 py-3 text-sm text-white placeholder:text-mist/30 outline-none focus:border-neon/40 transition-colors"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={e => setClinicIdentity(p => ({ ...p, phone: e.target.value }))}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={e => setClinicIdentity(p => ({ ...p, email: e.target.value }))}
              ...
              <div>
                ...
                <input
                  id="clinicRegInput"
                  type="text"
                  value={clinicIdentity.registrationNumber}
                  onChange={e => setClinicIdentity(p => ({ ...p, registrationNumber: e.target.value }))}
              ...
              <div>
                ...
                <input
                  id="clinicQualInput"
                  type="text"
                  value={clinicIdentity.qualification}
                  onChange={e => setClinicIdentity(p => ({ ...p, qualification: e.target.value }))}
              ...
              <div className="sm:col-span-2">
                ...
                <textarea
                  id="clinicAddressInput"
                  rows={2}
                  value={clinicIdentity.clinicAddress}
                  onChange={e => setClinicIdentity(p => ({ ...p, clinicAddress: e.target.value }))}
                  placeholder="Street, City, State, PIN"
                  className="w-full resize-none rounded-2xl border border-wire/15 bg-white/[0.04] px-4 py-3 text-sm text-white placeholder:text-mist/30 outline-none focus:border-neon/40 transition-colors"
                />
              </div>
            </div>

            {/* Preview strip */}
            {clinicIdentity.clinicName && (
              <div className="rounded-2xl border border-wire/8 bg-slate-950/30 px-4 py-3">
                <p className="text-[10px] uppercase tracking-[0.2em] text-mist mb-2">PDF Header Preview</p>
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="text-sm font-bold text-white leading-tight">{clinicIdentity.clinicName}</p>
                    {(clinicIdentity.qualification || props.doctorName) && (
                      <p className="text-xs text-mist mt-0.5">{props.doctorName}{clinicIdentity.qualification ? ` · ${clinicIdentity.qualification}` : ''}</p>
                    )}
                    {clinicIdentity.clinicAddress && <p className="text-[11px] text-mist/50 mt-0.5">{clinicIdentity.clinicAddress}</p>}
                  </div>
                  <div className="text-right text-[11px] text-mist/60 leading-relaxed">
                    {clinicIdentity.phone && <p>{clinicIdentity.phone}</p>}
                    {clinicIdentity.email && <p>{clinicIdentity.email}</p>}
                    {clinicIdentity.registrationNumber && <p>Reg: {clinicIdentity.registrationNumber}</p>}
                  </div>
                </div>
              </div>
            )}
            ...
            <button
              type="button"
              id="saveClinicIdentityBtn"
              onClick={saveClinicIdentity}
              disabled={clinicIdentitySaving}
              className="flex items-center gap-2 rounded-2xl border border-neon/25 bg-neon/[0.08] px-5 py-2.5 text-sm font-semibold text-neon transition-all duration-200 hover:bg-neon/[0.14] disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {clinicIdentitySaving ? (
                <span className="h-3.5 w-3.5 animate-spin rounded-full border-2 border-neon/30 border-t-neon" />
              ) : null}
              {clinicIdentitySaving ? 'Saving…' : 'Save Clinic Identity'}
            </button>
          </div>
        </PlaceholderBlock>
