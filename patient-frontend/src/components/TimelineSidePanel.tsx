...
function MetaTile({ label, value, border }: { label: string; value: string; border?: boolean }) {
  return (
    <div className={cn("px-4 py-3 flex-1", border && "border-l border-white/5")}>
      <p className="text-[9px] font-black uppercase tracking-[0.2em] text-mist/60 mb-1">{label}</p>
...
function ClinicalNoteRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex flex-col md:flex-row md:items-start gap-2 md:gap-8 py-4 border-t border-white/5">
      <span className="w-40 shrink-0 text-[10px] font-black uppercase tracking-widest text-mist/60 mt-1">{label}</span>
        ...
        >
          ...
          <header className="p-8 border-b border-white/5 flex items-start justify-between shrink-0 bg-gradient-to-b from-white/[0.02] to-transparent">
            <div>
              <div className="flex items-center gap-3 mb-2">
                <span className="text-[10px] font-black uppercase tracking-[0.4em] text-neon">Meiosis Clinical Portal</span>
                <span className={cn(
              ...
              <h2 className="text-3xl font-black text-white tracking-tight leading-none mb-1">{entry.type}</h2>
          ...
          <div className="flex-1 overflow-y-auto scroll-skin p-8 space-y-10">
              ...
              <section>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left">
                    <thead>
                      <tr className="text-[10px] font-black uppercase tracking-widest text-mist/60">
                ...
                <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
                    {[
                        ...
                        <div key={v.label} className="p-4 rounded-2xl bg-white/[0.02] border border-white/5 flex flex-col items-center">
                            <p className="text-[9px] font-black text-mist/40 uppercase mb-2">{v.label}</p>
                            <p className="text-sm font-black text-white">{v.value || 'NIL'}</p>
                        </div>
                    ))}
                </div>
            </section>
            ...
            <div className="flex items-center justify-between pt-4 opacity-40">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-[9px] font-black uppercase tracking-widest text-white">Encrypted Record Integrity Verified</span>
                </div>
          ...
          <div className="modal-actions p-6 border-t border-white/5 bg-white/[0.02] shrink-0 flex gap-4">
            <button 
              ...
              className="flex-1 bg-neon text-[#0A1118] font-black uppercase tracking-[0.2em] text-[10px] py-4 rounded-2xl flex items-center justify-center gap-2 hover:bg-neon/90 transition-all disabled:opacity-50"
            >
              {isGenerating ? 'Generating...' : <><Download className="w-4 h-4" /> Download Digital Rx</>}
            </button>
            <button onClick={onClose} className="flex-1 bg-white/5 text-white font-black uppercase tracking-[0.2em] text-[10px] py-4 rounded-2xl hover:bg-white/10 transition-all">
              Close Record
            </button>
          </div>
        </div>
